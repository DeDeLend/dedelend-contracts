...
const AmountRequired_1 = require("@opengsn/common/dist/AmountRequired");
const Environments_1 = require("@opengsn/common/dist/Environments");
...
class RelayServer extends events_1.EventEmitter {
    constructor(config, transactionManager, dependencies) {
        ...
        this.trustedPaymastersGasAndDataLimits = new Map();
        this.logger = dependencies.logger;
        this.lastScannedBlock = (_a = config.coldRestartLogsFromBlock) !== null && _a !== void 0 ? _a : 0;
        this.versionManager = new VersionsManager_1.VersionsManager(Version_1.gsnRuntimeVersion, Version_1.gsnRequiredVersion);
    ...
    statsHandler() {
        ...
        const now = Date.now();
        const statsResponse = Object.assign(Object.assign({}, this.readinessInfo), { totalUptime: now - this.readinessInfo.runningSince });
        if (this.isReady()) {
    ...
    validateInput(req, currentBlockNumber) {
        ...
        if (this.minGasPrice > requestGasPrice || parseInt(this.config.maxGasPrice) < requestGasPrice) {
            throw new Error(`gasPrice given ${requestGasPrice} not in range : [${this.minGasPrice}, ${this.config.maxGasPrice}]`);
        }
        ...
        if (expiredInBlocks < this.config.requestMinValidBlocks) {
            throw new Error(`Request expired (or too close): expired in ${expiredInBlocks} blocks, we expect it to be valid for ${this.config.requestMinValidBlocks}`);
        }
    }
    validateFees(req) {
    ...
    async validatePaymasterGasAndDataLimits(req) {
        ...
        const msgData = this.relayHubContract.contract.methods.relayCall(acceptanceBudget, req.relayRequest, req.metadata.signature, req.metadata.approvalData, dummyBlockGas).encodeABI();
        const msgDataLength = ethereumjs_util_1.toBuffer(msgData).length;
        ...
        if (gasAndDataLimits == null) {
            ...
            if (paymasterAcceptanceBudget + msgDataGasCostInsideTransaction > acceptanceBudget) {
                if (!this._isTrustedPaymaster(paymaster)) {
                    throw new Error(`paymaster acceptance budget + msg.data gas cost too high. given: ${paymasterAcceptanceBudget + msgDataGasCostInsideTransaction} max allowed: ${this.config.maxAcceptanceBudget}`);
                }
        ...
        const tmpMaxPossibleGas = Utils_1.calculateTransactionMaxPossibleGas({
            gasAndDataLimits,
            hubOverhead,
    ...
    async validateViewCallSucceeds(req, maxAcceptanceBudget, maxPossibleGas) {
        const method = this.relayHubContract.contract.methods.relayCall(maxAcceptanceBudget, req.relayRequest, req.metadata.signature, req.metadata.approvalData, maxPossibleGas);
...
`);
    ...
    async intervalHandler() {
        ...
        return this.contractInteractor.getBlockNumber()
            ...
            if (blockNumber < this.config.coldRestartLogsFromBlock) {
                throw new Error(`Cannot start relay worker with coldRestartLogsFromBlock=${this.config.coldRestartLogsFromBlock} when "latest" block returned is ${blockNumber}`);
            }
    ...
    async _initTrustedPaymasters(paymasters = []) {
        ...
        for (const paymasterAddress of paymasters) {
            ...
            const gasAndDataLimits = await paymaster.getGasAndDataLimits().catch((e) => {
    ...
    _getPaymasterMaxAcceptanceBudget(paymaster) {
        const limits = this.trustedPaymastersGasAndDataLimits.get(paymaster === null || paymaster === void 0 ? void 0 : paymaster.toLowerCase());
        if (limits != null) {
...
`);
    ...
    async _shouldRegisterAgain(currentBlock, hubEventsSinceLastScan) {
        ...
        const isPendingRegistration = await this.txStoreManager.isActionPending(StoredTransaction_1.ServerAction.REGISTER_SERVER);
        const isPendingActivity = isPendingRegistration || await this.txStoreManager.isActionPending(StoredTransaction_1.ServerAction.RELAY_CALL);
        const registrationExpired = this.config.registrationBlockRate !== 0 &&
            (currentBlock - latestRegisterTxBlockNumber >= this.config.registrationBlockRate) &&
            !isPendingRegistration;
        const activityExpired = this.config.activityBlockRate !== 0 &&
        ...
        if (!registrationExpired) {
            this.logger.debug(`_shouldRegisterAgain registrationExpired=${registrationExpired} currentBlock=${currentBlock} latestTxBlockNumber=${latestRegisterTxBlockNumber} registrationBlockRate=${this.config.registrationBlockRate}`);
        }
        if (!activityExpired) {
            this.logger.debug(`_shouldRegisterAgain activityExpired=${activityExpired} currentBlock=${currentBlock} latestTxBlockNumber=${latestTxBlockNumber} activityBlockRate=${this.config.activityBlockRate}`);
        }
    ...
    async handlePastHubEvents(currentBlockNumber, hubEventsSinceLastScan) {
        for (const event of hubEventsSinceLastScan) {
            switch (event.event) {
                case GSNContractsDataTypes_1.TransactionRejectedByPaymaster:
                    ...
                    break;
                case GSNContractsDataTypes_1.TransactionRelayed:
    ...
    _getLatestRegisterTxBlockNumber() {
        var _a, _b;
        return (_b = (_a = this.registrationManager.lastMinedRegisterTransaction) === null || _a === void 0 ? void 0 : _a.blockNumber) !== null && _b !== void 0 ? _b : -1;
    }
    ...
    _isBlacklistedPaymaster(paymaster) {
