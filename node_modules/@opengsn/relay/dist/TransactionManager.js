...
class TransactionManager {
    ...
    printBoostedTransactionLog(txHash, creationBlockNumber, gasPrice, isMaxGasPriceReached) {
        const gasPriceHumanReadableOld = new ethval_1.default(gasPrice).toGwei().toFixed(4);
        this.logger.info(`Boosting stale transaction:
hash         | ${txHash}
gasPrice     | ${gasPrice} (${gasPriceHumanReadableOld} gwei) ${isMaxGasPriceReached ? chalk_1.default.red('(MAX GAS PRICE REACHED)') : ''}
...
`);
    ...
    printSendTransactionLog(transaction, from) {
        ...
        const valueHumanReadable = new ethval_1.default(valueString).toEth().toFixed(4);
        const gasPriceHumanReadable = new ethval_1.default(gasPriceString).toGwei().toFixed(4);
...
`);
    ...
    async sendTransaction({ signer, method, destination, value = '0x', gasLimit, gasPrice, creationBlockNumber, serverAction }) {
        ...
        const _gasPrice = parseInt(gasPrice !== null && gasPrice !== void 0 ? gasPrice : await this.gasPriceFetcher.getGasPrice());
        const releaseMutex = await this.nonceMutex.acquire();
        if (Utils_1.isSameAddress(destination, Constants_1.constants.ZERO_ADDRESS)) {
        ...
        try {
            ...
            const txToSign = new tx_1.Transaction({
                to: destination,
                value: value,
                gasLimit,
                gasPrice: _gasPrice,
            ...
            storedTx = StoredTransaction_1.createStoredTransaction(signedTransaction.signedEthJsTx, metadata);
            this.nonces[signer]++;
    ...
    async updateTransactionWithMinedBlock(tx, minedBlockNumber) {
        const storedTx = Object.assign({}, tx, { minedBlockNumber });
        await this.txStoreManager.putTx(storedTx, true);
    }
    async updateTransactionWithAttempt(txToUpdate, tx, currentBlock) {
        const metadata = {
        ...
        const storedTx = StoredTransaction_1.createStoredTransaction(txToUpdate, metadata);
        await this.txStoreManager.putTx(storedTx, true);
        return storedTx;
    }
    ...
    _resolveNewGasPrice(oldGasPrice, minGasPrice) {
    ...
    async removeConfirmedTransactions(blockNumber) {
        ...
        for (const transaction of sortedTxs) {
            const nonce = (_a = nonces.get(transaction.from)) !== null && _a !== void 0 ? _a : await this.contractInteractor.getTransactionCount(transaction.from, blockNumber - this.config.confirmationsNeeded);
            ...
            if (transaction.nonce < nonce) {
                ...
                await this.txStoreManager.removeTxsUntilNonce(transaction.from, nonce);
                continue;
            }
     ...
     */
    async boostUnderpricedPendingTransactionsForSigner(signer, currentBlockHeight, minGasPrice) {
        var _a;
